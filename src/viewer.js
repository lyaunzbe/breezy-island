...
export default function(opt, canvasTexture) {
  ...
  materialRTT = new THREE.ShaderMaterial( {
  	uniforms: { "level":{ type: "f", value: 0.0 },"time": { type: "f", value: 0.0 },   "resolution"  : { type: "v2", value: new THREE.Vector2(window.innerWidth, window.innerHeight) } },
  	vertexShader: glslify('./shaders/bork.vert'),
  	fragmentShader: glslify('./shaders/bork.frag')
	});
  ...
  function fboDraw () {
    ...
    fbMaterial.fboCollection.forEach(function (fbo) {
      ...
      fbo.mat.uniforms.mouse.value = new THREE.Vector2(Math.sin(noise.getVal(smoothedLevel)*noise.getVal(smoothedLevel)) *2.0, Math.cos(noise.getVal(smoothedLevel)*noise.getVal(smoothedLevel))*2.0);
      // fbo.mat.uniforms.mouse.value = new THREE.Vector2(Math.sin(time)*1.5, Math.sin(time)*1.5);
